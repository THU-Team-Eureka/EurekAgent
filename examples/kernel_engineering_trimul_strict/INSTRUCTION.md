...
- You must use triton 3.3.1 and these kernels will be run on an NVIDIA A100-SXM4-80GB GPU.
...
- Initial code is provided in `initial.py`. You can use it as a reference or starting point.
- To evaluate your kernel, we provide an evaluator that runs the **unmodified TTT-Discover evaluation pipeline** (18 correctness tests + 7 benchmark cases, geometric mean of CUDA event timings). NEVER MODIFY THE EVALUATOR.
- Submission format: see `SUBMISSION_FORMAT.md`.
- Best known result on A100: **2198 us** (TTT-Discover). Best human: 4531 us. PyTorch baseline (the `initial.py` provided): ~20700 us on A100-SXM4-80GB.

## HINTS and RECOMMENDATIONS

Useful directions:
1. Fuse LayerNorm with the subsequent linear projections to reduce memory traffic.
2. Fuse the gating (sigmoid) with projection to avoid storing intermediate tensors.
3. Use mixed precision (FP16/BF16) for the expensive pairwise multiplication (einsum/bmm).
4. Fuse the output LayerNorm, out-gate multiplication, and final linear projection.
5. Choose block sizes that balance parallelism with shared memory usage (A100: 164 KB shared mem / SM, 108 SMs).
6. Consider using `torch.bmm` for the pairwise multiplication if a pure Triton implementation is not faster.

Be creative but prioritize correctness first, then optimize for speed.

Recommendations:
You can keep a note in `NOTES.md` to record your thoughts and ideas, and improvements each iteration. Save program iterations in an `iterations/` directory if helpful.
